...
interface DevLogTypeWithNickname extends devLogType {
    nickname?: { nickname: string }
}

interface DevLogDashboardProps {
    devLogList: DevLogTypeWithNickname[]
}

export default function DevLogDashboard({
    devLogList
}: DevLogDashboardProps) {
    const router = useRouter()
    const theme = useTheme()
    const isMobile = useMediaQuery(theme.breakpoints.down('md'))
    const { addQueryString } = useQueryString()

    const [selectedDevLog, setSelectedDevLog] = useState<DevLogTypeWithNickname | null>(null)
    ...
    const handleClickDevLog = useCallback((devLog: DevLogTypeWithNickname) => {
        if (!devLog.pk) return
    ...
    return (
        <div className='w-full h-full flex flex-col relative'>
            ...
            <div className='w-full h-10 relative flex'>
                <HomeIcon
                    className="cursor-pointer absolute top-0 right-0 size-4"
                    onClick={RouteToMyDevBlog}
                />
            </div>
            ...
            <div className='w-full h-full flex'>
                ...
                <div className='h-[calc(100vh-100px)] pb-[200px] overflow-y-scroll custom-scrollbar scrollbar-thin scrollbar-left pl-4'>
                    <div className={cn(
                        ...
                        {devLogList.map((devLog: DevLogTypeWithNickname) => (
...
export function DevLogDashboardItem({
    devLog,
    onClick
}: {
    devLog: DevLogTypeWithNickname | devLogType
    onClick: () => void
}) {
    return (
        <div className='w-full overflow-hidden flex-shrink-0 bg-transparent border border-neutral-800 rounded-lg px-4 py-2 cursor-pointer hover:border-neutral-400 transition-all duration-100 flex flex-col gap-2' onClick={onClick}>
            <h2 className='text-lg font-bold line-clamp-2'>{devLog.title}</h2>
            ...
            <div className='flex flex-col'>
                ...
                <p className='text-sm text-neutral-600 mt-[-4px]'>{dayjs(devLog?.updated_at ?? devLog?.created_at).fromNow()}</p>
            </div>
        </div>
    )
}