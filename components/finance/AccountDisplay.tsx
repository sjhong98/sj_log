...
export default function AccountDisplay({
  ...
  const handleDeleteAccount = useCallback(
    async (account: FinanceAccountType) => {
              ...
              {accounts
                ...
                .map((account, i) => {
                  return (
                    ...
                    >
                      <HyperText initial={''} className={'text-sm text-green-500 mr-4'}>
                        {account.title}
                      </HyperText>
                      <Row className={'text-3xl text-green-500 mt-[-12px]'}>
                        ...
                        />
                        <IconCurrencyWon className={'text-green-500 mt-3 w-4 h-4'} />
                      </Row>
                    </Column>
                  )
                })}
                ...
                .map((account, i) => {
                  return (
                    <Column
                      ...
                      onClick={() => handleOpenAccountDetail(account)}
                    >
                      <HyperText className={'text-sm text-green-500 mr-4'}>{account.title}</HyperText>
                      <Row className={'text-2xl font2 text-green-500 mt-[-12px]'}>
                        ...
                        />
                        <IconCurrencyWon className={'text-green-500 mt-[6px] w-4 h-4'} />
                      </Row>
                    </Column>
                  )
                })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">{selectedAccount.amount?.toLocaleString()}원</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">메모</label>
                    <p className="text-lg text-gray-800">{selectedAccount.note || '메모 없음'}</p>
                  </div>
                </div>
              </div>
                          ...
                          <div className="flex items-center space-x-3">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${
                                log.type === 'income' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                              }`}
                            >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">계좌 삭제 확인</h3>
                <p className="text-sm text-gray-500 mb-6">
                  <strong>&ldquo;{selectedAccount.title}&rdquo;</strong> 계좌를 삭제하시겠습니까?
                  <br />이 작업은 되돌릴 수 없으며, 관련된 모든 거래 내역도 함께 삭제됩니다.
                </p>
