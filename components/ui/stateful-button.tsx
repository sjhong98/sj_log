...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  hiddenSuccessIcon?: boolean;
}

export const Button = ({ className, children, hiddenSuccessIcon = false, ...props }: ButtonProps) => {
  ...
  const animateSuccess = async (result: any) => {
    await animate(
      ".loader",
      {
        width: "0px",
    ...
    if (hiddenSuccessIcon || !result) return;
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    try {
      ...
      await animateSuccess(result);
    } catch (error) {
      console.error('Stateful button error:', error);
    }
  };
