...
export default function Page() {
    ...
    return (
      ...
      nameList.map((name) => (
        ...
        >
          ...
          <Typography variant="subtitle1" className="!text-[12px] text-stone-500 line-clamp-1">
            {name?.subname || name.subname !== '' ? name.subname : ''}
          </Typography>
          ...
          {name.secretDescription !== '' && (
            <div className="absolute top-2 right-[8px] rounded-full bg-red-600 w-1 h-1" />
          )}
        </Column>
      ))
    )
  ...
  const renderNameListGroupByTags = useMemo(() => {
    return nameListGroupByTags.map((nameListGroup: any, index: number) => (
      <Column key={index} className="w-full">
        <Typography variant="body1">#{nameListGroup.tag.name ?? '-'}</Typography>

        <Row gap={4} className="flex-wrap overflow-y-auto">
          {nameListGroup.name.map((name: any) => (
            ...
            >
              ...
              {name.description !== '' && (
                <div className="absolute top-2 right-[20px] rounded-full bg-green-600 w-1 h-1" />
              )}
              {name.images !== '' && <div className="absolute top-2 right-[14px] rounded-full bg-stone-600 w-1 h-1" />}
              {name.secretDescription !== '' && (
                <div className="absolute top-2 right-[8px] rounded-full bg-red-600 w-1 h-1" />
  ...
  return (
    <Row gap={16} className="w-full">
      {/* Name List Area */}
      <Column gap={8} className="flex-[1] w-full">
        <Row gap={4}>
          <Row
            fullWidth
            gap={4}
        ...
        <Row gap={4} className="flex-wrap overflow-y-auto max-h-[calc(100vh-200px)] custom-scrollbar">
          {currentFilterIndex === 0 ? (
            <>
              ...
              {renderNameList}
              <Row fullWidth className="justify-center">
                <Typography className="text-stone-700 !text-[14px] font-light">{`- Total ${total} names -`}</Typography>
              </Row>
            </>
          ) : (
            <>
      ...
      {/* Name Detail Area */}
      {selectedNameDetail && (
        <Column gap={4} className="flex-[1.2] w-full p-2 rounded-xl bg-stone-900 h-fit">
          <Row gap={4} className="w-full bg-stone-900 rounded-md ">
            <Column
          ...
          <Row gap={4} fullWidth>
            <Row className="flex-[3] bg-[#010101] rounded-md items-center px-3">
              <Autocomplete
                ...
                className="ml-[-2px]"
                options={
                  tagInput.trim() && !tagOptions.find((option) => option.name === tagInput.trim())
                ...
                renderOption={(props, option) => (
                  <Box component="li" {...props} key={typeof option === 'string' ? option : option.pk}>
                    <Typography
                      variant="body2"
                      sx={{
                        color: typeof option === 'string' && option.includes('새로운 태그 추가') ? '#4ade80' : '#fff',
                      }}
                    >
      ...
      <Dialog open={showSecretModal} onClose={handleSecretModalClose}>
