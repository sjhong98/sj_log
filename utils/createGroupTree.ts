...
// parsing된 devLogGroup으로 Tree 구조 생성
export default function createGroupTree(allGroupList: devLogGroupType[]) {
  try {
    if(!Array.isArray(allGroupList)) {
      throw new Error('Given group list is not an array')
    }
    ...
    console.error(e)
    throw new Error(e instanceof Error ? e.message : 'Failed to create group tree')
  }
}
